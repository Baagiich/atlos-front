<template>
  <Toolbar
    :actions="['add']"
    :breadcrumb="breadcrumb"
    :is-loading="isLoading"
    @add="goToCreatePage"
  />

  <v-container fluid>
    <v-alert v-if="deleted" type="success" class="mb-4" closable="true">
      {{ $t("itemDeleted", [deleted["@id"]]) }}
    </v-alert>
    <v-alert v-if="mercureDeleted" type="success" class="mb-4" closable="true">
      {{ $t("itemDeletedByAnotherUser", [mercureDeleted["@id"]]) }}
    </v-alert>

    <v-alert v-if="error" type="error" class="mb-4" closable="true">
      {{ error }}
    </v-alert>


    <v-data-table-server
      :headers="headers"
      :items="items"
      :items-length="totalItems"
      :loading="isLoading"
      :items-per-page="items.length"
      @update:page="updatePage"
      @update:sortBy="updateOrder"
    >
      <template #item.actions="{ item }">
        <ActionCell
          :actions="['show', 'update', 'delete']"
          @show="goToShowPage(item)"
          @update="goToUpdatePage(item)"
          @delete="deleteItem(item)"
        />
      </template>

      <template #item.@id="{ item }">
        <router-link
          :to="{ name: 'AddressShow', params: { id: item['@id'] } }"
        >
          {{ item["@id"] }}
        </router-link>
      </template>

      <template #item.city="{ item }">
        <router-link
          v-if="router.hasRoute('CityShow')"
          :to="{ name: 'CityShow', params: { id: item.city } }"
        >
          {{ item.city }}
        </router-link>

        <p v-else>
          {{ item.city }}
        </p>
      </template>
    </v-data-table-server>
  </v-container>
</template>

<script setup lang="ts">
import { ref, onBeforeUnmount } from "vue";
import { useI18n } from "vue-i18n";
import { useRouter } from "vue-router";
import { storeToRefs } from "pinia";
import { useAddressListStore } from "@/store/address/list";
import { useAddressDeleteStore } from "@/store/address/delete";
import Toolbar from "@/components/common/Toolbar.vue";
import ActionCell from "@/components/common/ActionCell.vue";
import { useMercureList } from "@/composables/mercureList";
import { useBreadcrumb } from "@/composables/breadcrumb";
import type { VuetifyOrder } from "@/types/list";
import type { Address } from "@/types/address";

const { t } = useI18n();
const router = useRouter();
const breadcrumb = useBreadcrumb();

const addressDeleteStore = useAddressDeleteStore();
const { deleted, mercureDeleted } = storeToRefs(addressDeleteStore);

const addressListStore = useAddressListStore();
const { items, totalItems, error, isLoading } = storeToRefs(addressListStore);

const page = ref("1");
const order = ref({});

async function sendRequest() {
  await addressListStore.getItems({
    page: page.value,
    order: order.value,
  });
}

useMercureList({ store: addressListStore, deleteStore: addressDeleteStore });

sendRequest();

const headers = [
  {
    title: t("actions"),
    key: "actions",
    sortable: false,
  },
  { title: t("id"), key: "@id" },
  {
    title: t("address.street"),
    key: "street",
    sortable: false,
  },
  {
    title: t("address.city"),
    key: "city",
    sortable: false,
  },
];

function updatePage(newPage: string) {
  page.value = newPage;

  sendRequest();
}

function updateOrder(newOrders: VuetifyOrder[]) {
  const newOrder = newOrders[0];
  order.value = { [newOrder.key]: newOrder.order };

  sendRequest();
}


function goToShowPage(item: Address) {
  router.push({
    name: "AddressShow",
    params: { id: item["@id"] },
  });
}

function goToCreatePage() {
  router.push({
    name: "AddressCreate",
  });
}

function goToUpdatePage(item: Address) {
  router.push({
    name: "AddressUpdate",
    params: { id: item["@id"] },
  });
}

async function deleteItem(item: Address) {
  await addressDeleteStore.deleteItem(item);

  sendRequest();
}

onBeforeUnmount(() => {
  addressDeleteStore.$reset();
});
</script>
