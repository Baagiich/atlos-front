    ...
    <v-row>
      ...
      <v-col cols="12" sm="6" md="6">
        <v-text-field
          ...
          :error="Boolean(violations?.vehicle)"
          :error-messages="violations?.vehicle"
          :label="$t('vehicleimage.vehicle')"
        >
          <template #append-inner>
            <v-icon
              style="cursor: pointer"
              @click.prevent.stop="item.vehicle = undefined"
            >
              mdi-close
            </v-icon>
          </template>
      ...
      <v-col cols="12" sm="6" md="6">
        <v-text-field
          ...
          :label="$t('vehicleimage.updatedAt')"
          type="date"
        >
          <template #append-inner>
            <v-icon
      ...
      <v-col cols="12" sm="6" md="6">
        <v-text-field
          ...
          :label="$t('vehicleimage.createdAt')"
          type="date"
        >
          <template #append-inner>
            <v-icon
...
const item: Ref<VehicleImage> = ref({});
...
const emit = defineEmits<{
  (e: "submit", item: VehicleImage): void;
}>();
