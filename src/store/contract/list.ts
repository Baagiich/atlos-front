...
export const useContractListStore = defineStore("contractList", {
  ...
  actions: {
    async getItems(params: ListParams) {
      ...
      try {
        const response = await api("contracts", { params });
        const data: PagedCollection<Contract> = await response.json();
    ...
    deleteItem(deletedItem: Contract) {
